...
sudo apt install -y \
  ...
  python3.12 python3.12-dev libacl1-dev libacl1 libssl-dev liblz4-dev libzstd-dev libxxhash-dev \
  build-essential pkg-config python3-pkgconfig libfuse3-dev fuse3 \
  \
  hpnssh-client \
...
sudo PIPX_HOME=/opt/pipx PIPX_BIN_DIR=/usr/local/bin \
  pipx install 'borgbackup[pyfuse3]==2.0.0b7' --suffix 2 --python python3.12
# Exact version because betas can have breaking changes.
# Future: use a PPA

sudo PIPX_HOME=/opt/pipx PIPX_BIN_DIR=/usr/local/bin \
  pipx install 'borgmatic==1.8.5' --suffix 2 --python python3.12
